...
<div x-show="viewMode==='review'&&(item.status==='pending'||item.status==='duplicate')" class="mt-3 flex flex-wrap items-center gap-2"><div class="flex rounded-xl bg-slate-100 p-1"><button @click="setDirection(item,'credit')" :class="item.direction==='credit'?'bg-emerald-600 text-white':'text-slate-500'" class="h-8 px-2 rounded-lg text-[9px] font-black">Ingreso +</button><button @click="setDirection(item,'debit')" :class="item.direction==='debit'?'bg-rose-600 text-white':'text-slate-500'" class="h-8 px-2 rounded-lg text-[9px] font-black">Egreso −</button></div><select x-model="item.suggested_action" @change="setSuggestedAction(item)" class="h-10 rounded-xl border bg-white px-2 text-[10px] font-black"><option value="transaction">Movimiento</option><option value="payment">Abono recibido</option><option value="transfer">Entre cuentas</option></select><div class="ml-auto flex gap-2"><button @click="openReview(item)" class="h-9 px-3 rounded-xl border text-[10px] font-black">Detalles</button><button @click="ignore(item)" class="h-9 px-3 rounded-xl bg-slate-100 text-[10px] font-black">Ignorar</button><button @click="quickApply(item)" class="h-9 px-3 rounded-xl bg-slate-900 text-white text-[10px] font-black" x-text="actionLabel(item.suggested_action)"></button></div></div>
...
<div class="grid grid-cols-2 gap-3"><label class="text-[10px] font-black">Flujo del dinero<select x-model="review.item.direction" class="mt-1 w-full h-10 rounded-xl border px-2 text-xs font-black"><option value="unknown">Sin determinar</option><option value="credit">Ingreso (+)</option><option value="debit">Egreso (−)</option></select></label><label class="text-[10px] font-black">Fecha<input type="datetime-local" x-model="review.item.movement_date_local" class="mt-1 w-full h-10 rounded-xl border px-2 text-[11px]"></label></div>
...
<div><p class="text-[10px] font-black mb-2">Acción contable</p><div class="grid grid-cols-3 gap-2"><template x-for="a in [{v:'payment',i:'payments',l:'Abono recibido'},{v:'transaction',i:'receipt_long',l:'Movimiento'},{v:'transfer',i:'sync_alt',l:'Entre cuentas'}]"><button @click="review.action=a.v" :class="review.action===a.v?'bg-emerald-600 text-white':'bg-slate-100 text-slate-600'" class="py-2.5 rounded-xl text-[9px] font-black"><span class="material-icons text-lg block" x-text="a.i"></span><span x-text="a.l"></span></button></template></div></div>
...
<div x-show="review.item.status==='duplicate'" class="p-3 rounded-xl bg-amber-50 border border-amber-200 text-[11px] text-amber-800 font-bold">Coincide con un movimiento importado. Confirma solo si verificaste que es distinto.</div><button @click="saveAndApply()" :disabled="review.loading||review.item.direction==='unknown'" class="w-full h-11 rounded-2xl bg-emerald-600 disabled:bg-slate-300 text-white font-black text-xs" x-text="review.loading?'Guardando...':review.item.direction==='unknown'?'Define Ingreso o Egreso':'Guardar y confirmar'"></button>
</div></aside>
